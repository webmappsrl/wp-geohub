...
function delete_all_pois_action()
{
    ...
    $transient_key = 'wm_delete_pois_list';
    $progress_key = 'wm_delete_pois_progress';
    ...
    foreach ($batch_posts as $post_id) {
        ...
        if (get_post($post_id)) {
            ...
            if ($result) {
                ...
                $errors[] = sprintf(__('Error deleting POI ID: %d', 'wm-package'), $post_id);
            }
        }
    }
    ...
    if ($is_complete) {
        ...
        if (!empty($errors)) {
            set_transient('wm_delete_pois_notification', sprintf(__('Deleted %d POIs. Some errors occurred.', 'wm-package'), $progress_data['total_deleted']), 60);
        } else {
            set_transient('wm_delete_pois_notification', sprintf(__('Successfully deleted %d POIs.', 'wm-package'), $progress_data['total_deleted']), 60);
        }
    }
    ...
    if (wp_doing_ajax()) {
        wp_send_json_success([
            'message' => $is_complete 
                ? sprintf(__('Deletion completed! Deleted %d POIs.', 'wm-package'), $progress_data['total_deleted'])
                : sprintf(__('Batch completed: %d/%d POIs (%d%%)', 'wm-package'), $next_offset, $total_posts, $progress_percent),
            'progress' => [
